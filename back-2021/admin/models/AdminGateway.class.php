...
/**
 * AdminGateway.class [ MODEL ADMIN ]
 * Respnsável por gerenciar os Gateways de pagamento no Admin do sistema!
 *
...
class AdminGateway {
    ...
    public function ExeUpdate($PostId, array $Data) {
        ...
        if ($this->Result):
            ...
            if (is_array($this->Data['logomarca'])):
                ...
                $uploadCapa->ImageSemData($this->Data['logomarca'], $this->Data['url'], '300', 'gateways');
            endif;

            if (isset($uploadCapa) && $uploadCapa->getResult()):
                $this->Data['logomarca'] = $uploadCapa->getResult();
                $this->Update();
            else:
    ...
    private function Update() {
        ...
        $Update->ExeUpdate(self::Entity, $this->Data, "WHERE id = :id", "id={$this->Post}");
        if ($Update->getResult()):
            $this->Error = ["O Gateway <b>{$this->Data['nome']}</b> foi atualizado com sucesso no sistema!", RM_ACCEPT];
            $this->Result = true;
        endif;
    }
