...
class EmailAdmin {
    ...
    private $DestinoCopia;
    private $DestinoCCopia;
    private $DestinoArquivo;
    private $DestinoArquivoNome;
    ...
    private function setMail() {
        ...
        $this->DestinoCopia = $this->Data['DestinoCopia'];
        $this->DestinoCCopia = $this->Data['DestinoCCopia'];
        $this->DestinoArquivo = $this->Data['DestinoArquivo'];
        $this->DestinoArquivoNome = $this->Data['DestinoArquivoNome'];
    ...
    private function Config() {
        ...
        $this->Mail->AddCC($this->DestinoCopia);
        $this->Mail->AddBCC($this->DestinoCCopia);
        $this->Mail->AddAttachment($this->DestinoArquivo,$this->DestinoArquivoNome);
    }

    //Envia o e-mail!
    private function sendMail() {
        if ($this->Mail->Send()):
            $this->Error = ['Obrigado por entrar em contato: Recebemos sua mensagem e estaremos respondendo em breve!', RM_ACCEPT];
            $this->Result = true;
        else:
