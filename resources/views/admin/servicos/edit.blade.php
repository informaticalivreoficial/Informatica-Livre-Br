...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1><i class="fas fa-search mr-2"></i>Editar Serviço</h1>
        </div>
        <div class="col-sm-6">
...
<form action="{{ route('servicos.update',['id' => $servico->id]) }}" method="post" enctype="multipart/form-data" autocomplete="off">
@csrf     
@method('PUT')     
<div class="row">            
<div class="col-12">
            ...
            <div class="tab-pane fade show active" id="custom-tabs-four-home" role="tabpanel" aria-labelledby="custom-tabs-four-home-tab">
                ...
                <div class="row mb-2"> 
                    <div class="col-12 col-md-6 col-lg-4"> 
                        <div class="form-group">
                            <label class="labelforms text-muted"><b>*Nome do Serviço</b> </label>
                            <input type="text" class="form-control" name="name" value="{{ old('name') ?? $servico->name }}">
                        </div>
                    </div> 
                    <div class="col-12 col-md-6 col-lg-5">   
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="headline" value="{{old('headline') ?? $servico->headline}}">
                        </div>                                                    
                    </div> 
                    <div class="col-12 col-sm-4 col-md-4 col-lg-3">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" {{ (old('status') == '1' ? 'selected' : ($servico->status == '1' ? 'selected' : '')) }}>Publicado</option>
                                <option value="0" {{ (old('status') == '0' ? 'selected' : ($servico->status == '0' ? 'selected' : '')) }}>Rascunho</option>
                            </select>
                        </div>
                    </div> 
                    <div class="col-12 col-sm-5 col-md-5 col-lg-4">
                        <div class="form-group">
                            <label class="labelforms text-muted"><b>*Categoria:</b> <a style="font-size:11px;" href="{{route('servicos-categorias.index')}}">(Criar categoria)</a></label>
                            <select name="categoria" class="form-control categoria">
                                @if(!empty($catServicos) && $catServicos->count() > 0)
                                    <option value="">Selecione a Categoria</option>
                                    @foreach($catServicos as $categoria)                                
                                        <optgroup label="{{ $categoria->titulo }}">  
                                            @if($categoria->children)
                                                @foreach($categoria->children as $subcategoria)
                                                    <option value="{{ $subcategoria->id }}" {{ (old('categoria') == $subcategoria->id ? 'selected' : ($servico->categoria == $subcategoria->id ? 'selected' : '')) }}>{{ $subcategoria->titulo }}</option>
                                                @endforeach
                                            @endif
                                        </optgroup>                                                                                       
                                    @endforeach
                    ...
                    <div class="col-12 col-sm-3 col-md-3 col-lg-3">
                        <div class="form-group">
                            ...
                            <select name="exibir" class="form-control">
                                <option value="1" {{ (old('exibir') == '1' ? 'selected' : ($servico->exibir == '1' ? 'selected' : '')) }}>Sim</option>
                                <option value="0" {{ (old('exibir') == '0' ? 'selected' : ($servico->exibir == '0' ? 'selected' : '')) }}>Não</option>
                            </select>
                        </div>
                    </div>                          
                </div>
                    ...
                    <div class="col-12"> 
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input id="tipounico" class="form-check-input" type="radio" value="1" name="tipo_pagamento" {{(old('tipo_pagamento') == '1' ? 'checked' : ($servico->tipo_pagamento == '1' ? 'checked' : ''))}}>
                                <label for="tipounico" class="form-check-label mr-5">Único</label>
                                <input id="tiporecorrente" class="form-check-input" type="radio" value="0" name="tipo_pagamento" {{(old('tipo_pagamento') == '0' ? 'checked' : ($servico->tipo_pagamento == '0' ? 'checked' : '') )}}>
                ...
                <div class="row mb-2">
                    <div class="col-12 col-sm-4 col-md-4 col-lg-2">
                        ...
                        <input type="text" class="form-control mask-money v" name="valor" value="{{ old('valor') ?? $servico->valor }}">
                    </div>
                    <div class="col-12 col-sm-4 col-md-4 col-lg-2">
                        <label class="labelforms text-muted"><b>Mensal</b></label>
                        <input type="text" class="form-control mask-money m" name="valor_mensal" value="{{ old('valor_mensal') ?? $servico->valor_mensal }}">
                    </div>
                    <div class="col-12 col-sm-4 col-md-4 col-lg-2">
                        <label class="labelforms text-muted"><b>Trimestral</b></label>
                        <input type="text" class="form-control mask-money t" name="valor_trimestral" value="{{ old('valor_trimestral') ?? $servico->valor_trimestral }}">
                    </div>
                    <div class="col-12 col-sm-4 col-md-4 col-lg-2">
                        <label class="labelforms text-muted"><b>Semestral</b></label>
                        <input type="text" class="form-control mask-money s" name="valor_semestral" value="{{ old('valor_semestral') ?? $servico->valor_semestral }}">
                    </div>
                    <div class="col-12 col-sm-4 col-md-4 col-lg-2">
                        <label class="labelforms text-muted"><b>Anual</b></label>
                        <input type="text" class="form-control mask-money a" name="valor_anual" value="{{ old('valor_anual') ?? $servico->valor_anual }}">
                    </div>
                    <div class="col-12 col-sm-4 col-md-4 col-lg-2">
                        <label class="labelforms text-muted"><b>Bi-anual</b></label>
                        <input type="text" class="form-control mask-money b" name="valor_bianual" value="{{ old('valor_bianual') ?? $servico->valor_bianual }}">
                    </div>
                </div>
                ...
                <div class="row mb-2">
                    <div class="col-12 mb-1"> 
                        <div class="form-group">
                            ...
                            <input id="tags_1" class="tags" rows="5" name="tags" value="{{ old('tags') ?? $servico->tags }}">
                        </div>
                    </div>
                    <div class="col-12">   
                        <label class="labelforms text-muted"><b>Descrição do servico</b></label>
                        <x-adminlte-text-editor name="content" v placeholder="Descrição do servico..." :config="$config">{{ old('content') ?? $servico->content }}</x-adminlte-text-editor>                                                                                     
                    </div>                        
                </div>
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-12 col-sm-12 col-md-6 col-lg-6">   
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control"  name="thumb_legenda" value="{{ old('thumb_legenda') ?? $servico->thumb_legenda }}">
                        </div>                                                    
                    </div>
                    <div class="col-sm-12">                                        
                        <div class="form-group">
                        ...
                        <div class="property_image">
                            ...
                            <div class="property_image_item">
                                ...
                                <div class="property_image_actions">
                                    <a href="javascript:void(0)" class="btn btn-xs {{ ($image->cover == true ? 'btn-success' : 'btn-default') }} icon-notext image-set-cover px-2" data-action="{{ route('servicos.imageSetCover', ['image' => $image->id]) }}"><i class="nav-icon fas fa-check"></i> </a>
                                    <a href="javascript:void(0)" class="btn btn-danger btn-xs image-remove px-2" data-action="{{ route('servicos.imageRemove', ['image' => $image->id]) }}"><i class="nav-icon fas fa-times"></i> </a>
                                </div>
                            </div>
                            @endforeach
                        </div>
